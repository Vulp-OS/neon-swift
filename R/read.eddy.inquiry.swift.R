read.eddy.inquiry <- function(dataType = NULL,
                              siteID = NULL,
                              startDate = NULL,
                              endDate = NULL,
                              sensor = NULL,
          ...
          if (siteID %in% allowed.SiteID) {
            ...
            if (dataType == "2min") {
              ...
              if (sensor %in% allowed.2min) {
                if (sensor == "ecse.mfm.pressures") {
                  sensor <- "ECSE.MFM.Pressures"
                }

                twomin.starttime <- Sys.time()
                if (silent == FALSE) {
                  base::message(paste0("Pull configured correctly: ", siteID, " ", dataType, " ", sensor, " data from ", startDate, " to ", endDate))
                }
                ...
                if (silent == FALSE) {
                  message(paste0("Downloading ", nrow(list.s3.2min), " files - ", size.pull, " MB"))
                }
                ...
                if (length(list.s3.2min$Key) > 0) {
                  ...
                  if (silent == FALSE) {
                    pb <- txtProgressBar(0, length(list.s3.2min$Key), style = 3)
                  }
            ...
            if (dataType == "CnC") {
              ...
              if (silent == FALSE) {
                base::message(paste0("Pull configured correctly: ", dataType, " ", siteID, " data from ", startDate, " to ", endDate))
              }
            ...
            if (dataType == "spanGas") {
              ...
              if (silent == FALSE) {
                message(paste0("Downloading ", nrow(list.s3.spanGas), " files - ", size.pull, " MB"))
              }
              ...
              if (length(list.s3.spanGas$Key) > 0) {
                ...
                if (silent == FALSE) {
                  pb <- txtProgressBar(0, length(list.s3.spanGas$Key), style = 3)
                }
            ...
            if (dataType == "cval") {
              if (is.null(sensor) == FALSE) {
                if (sensor %in% allowed.cval) {
                  ...
                  if (silent == FALSE) {
                    message(paste0("Downloading ", nrow(list.s3.cval), " files - ", size.pull, " MB"))
                  }
                  ...
                  if (length(list.s3.cval$Key) > 0) {
                    ...
                    if (silent == FALSE) {
                      pb <- txtProgressBar(0, length(list.s3.cval$Key), style = 3)
                    }
                    ...
                    for (file in 1:length(list.s3.cval$Key)) {
                      ...
                      pull.file$uid <- paste0(
                        as.Date(list.s3.cval$StartDateTime[file]), "_",
            ...
            if (dataType == "cval.stats") {
              if (is.null(sensor) == FALSE) {
                if (sensor %in% allowed.cval) {
                  ...
                  if (silent == FALSE) {
                    message(paste0("Downloading ", nrow(list.s3.cval.stats), " files - ", size.pull, " MB"))
                  }
                  ...
                  if (length(list.s3.cval.stats$Key) > 0) {
                    cval.join.starttime <- Sys.time()
                    if (silent == FALSE) {
                      pb <- txtProgressBar(0, length(list.s3.cval.stats$Key), style = 3)
                    }
                    ...
                    for (file in 1:length(list.s3.cval.stats$Key)) {
                      ...
                      pull.file$uid <- paste0(
                        as.Date(list.s3.cval.stats$StartDateTime[file]), "_",
                        base::substr(rand.seed$.Random.seed[[file]], start = 1, stop = 4)
                      )
