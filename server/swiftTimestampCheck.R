    ...
    reactive_timestamp_data <- shiny::reactive({
      ...
      if (base::nrow(timestamp_data) > 0) {
        ...
        timestamp_data_named <- busted_sensors %>%
          ...
          dplyr::mutate(`Raw Time Difference` = Actual_Time_Difference) %>%
          dplyr::select(-Actual_Time_Difference) %>%
          dplyr::mutate(`Median Site Time Difference` = median_site_time_diff) %>%
          dplyr::select(-median_site_time_diff) %>%
          dplyr::mutate(`Calculated Timestamp Drift` = timestamp_drift) %>%
          dplyr::select(-timestamp_drift) %>%
          dplyr::mutate(SurveyTime = TimeStamp) %>%
