shiny::observeEvent(input$menu, {
  if (input$menu == "swft_wet_dep_tab") {
    ...
    reactive_wet_dep_plot_2 <- shiny::reactive({
      ggplot(
        reactive_wet_dep_data() %>% dplyr::mutate(days = round(sampleduration / 24, 0)) %>% dplyr::mutate(SiteID = siteid),
    ...
    reactive_wet_dep_plot_3 <- shiny::reactive({
      ggplot(
        reactive_wet_dep_data() %>% dplyr::mutate(`Isotope Subsample Mass` = round(isosubsamplebottleendmassmeas, 2)) %>% dplyr::mutate(SiteID = siteid),
    ...
    reactive_wet_dep_plot_4 <- shiny::reactive({
      ggplot(
        reactive_wet_dep_data() %>% dplyr::mutate(`Chem Subsample Mass` = round(chemsubsamplebottleendmassmeas, 2)) %>% dplyr::mutate(SiteID = siteid),
